...
export const LetterTopicSelector: React.FC<LetterTopicSelectorProps> = ({
  ...
  return (
    <div className="space-y-9">
      {/* 1. 대상 대분류 — 경조사 대분류(경사/조사)와 동일한 카드 크기/폰트
          (p-4 rounded-2xl, text-sm sm:text-base font-bold, 체크 원). 실제
          등록된 수신자 관계와는 별개로, "이 편지가 그려야 할 대상 성격"만
      ...
      <div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 font-sans">
          {RELATION_GROUPS.map((group) => {
            ...
            return (
              ...
              >
                <span className="font-bold text-sm sm:text-base text-[#3D2B31]">{group.label}</span>
                {isActiveGroup && (
      ...
      {/* 2. 세부 대상 (중분류) — 경조사의 "주요 항목"과 같은 자리/크기.
          대분류 안에 세부 유형이 여럿일 때만(연인처럼 하나뿐이면 생략) 별도
          섹션으로 나타난다. */}
      ...
      {/* 3. 편지 주제 (소분류) — 경조사의 세부 상황 태그와 동일한 알약형 태그.
          대상(대분류)을 골라야만 나타나는 진짜 하위 단계. 대상을 안 고르면 이
          단계 자체가 안 보이지만, 추가 요청사항 자유 텍스트만으로 생성하는
          경로는 그대로 남아있다. */}
      {audienceType && (
        <div>
