...
import { captureCardAsPngDataUrl, captureCardAsBlob } from '../../lib/cardCapture';
import {
  Relationship,
  LetterCategory,
  MessageFormat,
...
export const DeliveryCard: React.FC<DeliveryCardProps> = ({
  ...
  // 수신자 등록은 선택사항 — 없으면 카드/공유 문구에 일반적인 호칭을 대신 쓴다.
  const displayName = relationship ? withHonorific(relationship.name) : '받는 분';
  ...
  const formattedShareText = `[${category} 멘트] ${displayName}께 전하는 마음\n-------------------\n${messageContent}\n-------------------\n- 경조사 멘트 AI 도우미에서 생성됨`;
  ...
  const handleDownloadCardImage = async () => {
    ...
    try {
      const dataUrl = await captureCardAsPngDataUrl(cardRef.current);
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: `To. ${displayName}께 전하는 ${category} 카드`,
          text: messageContent,
          files: [file],
        });
      } else {
            ...
            <div className="space-y-1.5 max-w-[85%]">
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-md border border-slate-200">
                <div className="p-3 bg-brand-50/60 border-b border-brand-100 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-brand-800 font-mono">To. {relationship?.name ?? '받는 분'}</span>
                </div>
