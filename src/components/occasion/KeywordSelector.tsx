...
interface KeywordSelectorProps {
  ...
  onSelectPrimaryKeyword: (pk: PromptKeyword | null) => void;
...
export const KeywordSelector: React.FC<KeywordSelectorProps> = ({
  ...
  const subList = primaryKeyword ? getSubKeywords(primaryKeyword.id) : [];
    ...
    <div className="space-y-9">
      ...
      <div>
        ...
        <p className="text-[11px] text-[#3D2B31]/50 -mt-1 mb-2.5">
          고르지 않아도 괜찮아요 — 아래 추가 요청사항만으로도 문구를 지을 수 있어요.
        </p>
        ...
        <div className="grid grid-cols-2 gap-3 font-sans">
          ...
          <button
            type="button"
            onClick={() => onSelectCategory(category === '경사' ? '미지정' : '경사')}
          ...
          <button
            type="button"
            onClick={() => onSelectCategory(category === '조사' ? '미지정' : '조사')}
            className={`p-4 rounded-2xl border text-left transition-all flex items-center justify-between cursor-pointer ${
              category === '조사'
      ...
      {/* 2. Primary Keyword Selector — 대분류를 아예 안 고르면(미지정) 목록이
          비어 있으므로 섹션 자체를 숨긴다. */}
            ...
            {primaryList.map((pk) => {
              ...
              return (
                <button
                  type="button"
                  key={pk.id}
                  onClick={() => onSelectPrimaryKeyword(isSelected ? null : pk)}
